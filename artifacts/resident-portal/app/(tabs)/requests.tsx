...
type StatusConfig = { label: string; bg: string; text: string; icon: 'clock' | 'refresh-cw' | 'check-circle' | 'help-circle' };

function statusConfig(estado: string | null): StatusConfig {
  switch (estado) {
    case 'pendiente':
      return { label: 'Pendiente', bg: 'rgba(255,140,80,0.15)', text: '#d4640a', icon: 'clock' };
    case 'en_proceso':
      return { label: 'En proceso', bg: 'rgba(24,71,204,0.12)', text: '#1847CC', icon: 'refresh-cw' };
    case 'resuelto':
      return { label: 'Resuelto', bg: 'rgba(62,148,113,0.15)', text: '#2e7a57', icon: 'check-circle' };
    default:
      return { label: estado ?? 'Sin estado', bg: 'rgba(90,106,133,0.12)', text: '#5A6A85', icon: 'help-circle' };
  }
}
...
export default function RequestsScreen() {
  ...
  useEffect(() => { void fetchIncidencias(); }, [fetchIncidencias]);
...
const styles = StyleSheet.create({
  ...
  newButton: {
    height: 50, borderRadius: 15, marginTop: 22,
