...
export type PaymentDataState = "unknown" | "ready";

export const RESIDENT_PAYMENT_SELECT =
  "id_pago,monto,estado,fecha_vencimiento,concepto";

const DATE_ONLY_PATTERN = /^\d{4}-\d{2}-\d{2}$/;

export function paymentDataState({
  loading,
  error,
}: {
  loading: boolean;
  ...
  return loading || error ? "unknown" : "ready";
}
...
export function getNextPendingPayment<T extends PaymentSummary>(
  payments: T[],
): T | undefined {
  ...
  for (const payment of payments) {
    ...
    if (
      !next ||
      parseResidentDate(payment.fecha_vencimiento).getTime() <
        parseResidentDate(next.fecha_vencimiento).getTime()
    ) {
      next = payment;
    }
  }
